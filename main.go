...
func init() {
	flag.StringVar(&port, "p", "8080", "listening port,defaut port is 8080")
	//中文乱码问题
	//字体设置必须在主窗口创建之前
	err := os.Setenv("FYNE_FONT", "./fonts/msyh.ttc") // 设置环境变量  // 取消环境变量 os.Unsetenv("FYNE_FONT")
	if err != nil {
		panic(err)
	}
	a = app.New()
...
func main() {
	...
	//GUI
	mainWindow = a.NewWindow("键鼠模拟服务")
	ip, err := util.GetOutBoundIP()
	if err != nil {
		panic(err)
	}
	ipLabel := widget.NewLabel(fmt.Sprintf("服务IP：http://%s", fmt.Sprintf("%s:%s", ip, port)))
